...
export function generateIncidentPrediction(deploymentId?: string): IncidentPrediction {
  ...
  const similarIncidents: SimilarIncident[] = Array.from({ length: Math.floor(Math.random() * 3) + 1 }, () => ({
    id: uuid(),
...
export function generateAnomalies(count: number = 5): AnomalyDetection[] {
  const metrics = [
    { name: 'response_time_p99', unit: 'ms', base: 200, variance: 500 },
    { name: 'error_rate', unit: '%', base: 0.5, variance: 15 },
    { name: 'cpu_usage', unit: '%', base: 45, variance: 40 },
    { name: 'memory_usage', unit: '%', base: 60, variance: 30 },
    { name: 'request_rate', unit: 'rps', base: 1500, variance: 3000 },
    { name: 'db_query_time', unit: 'ms', base: 50, variance: 200 },
    { name: 'kafka_consumer_lag', unit: 'msgs', base: 100, variance: 5000 },
    { name: 'gc_pause_time', unit: 'ms', base: 20, variance: 100 },
  ];
  ...
  return Array.from({ length: count }, () => {
    ...
    const severity: 'critical' | 'warning' | 'info' = deviation > 200 ? 'critical' : deviation > 100 ? 'warning' : 'info';
...
export function generateAlertClusters(count: number = 4): AlertCluster[] {
  ...
  return Array.from({ length: count }, () => {
    ...
    const severity = alertCount > 25 ? 'critical' : alertCount > 15 ? 'high' : alertCount > 8 ? 'medium' : 'low';
